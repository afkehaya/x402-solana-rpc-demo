...
import { Facilitator, FacilitatorConfig } from "../x402/facilitator";
import config from "../x402/config.json";

const app = express();
app.use(bodyParser.json());

// RPC endpoint: prefer Pulumi-deployed node, fallback to public Devnet RPC
...
const solanaRpcUrl = config.network === "devnet"
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // err is unknown, cast to any for error message
